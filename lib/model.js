...
const DatabaseFetcher = require('./helpers/database-fetcher');
...
class Model {
	...
	getDb() {
		...
		if(this.databaseFetcher)
			this.databaseFetcher = this.session ? this.session.getSessionInstance(DatabaseFetcher) : new DatabaseFetcher();

		return this.databaseFetcher.getDb(this.databaseKey, !!this.useReadDB);
	}

	/**
	 * @param {DBDriver} dbDriver A database driver
	 * @param {string} methodName A method name to check for existence in the DB Driver
	 * @param {boolean} rejects Determinate if should reject when Driver does not have implemented the method
	 * @throws {ModelError} If method is not present in dbDriver
	 */
	validateMethodImplemented(dbDriver, methodName, rejects = true) {
	...
	async getPaged(data = {}, callback) {
		...
		if(this.validateMethodImplemented(db, 'getPaged', false))
