...
class Model {
	...
	get logHelper() {
		...
		this._logHelper = new LogHelper({
			...
			coreResolver: () => this.dispatcher.isCoreDatabase(this.databaseKey),
	...
	async insert(item) {
		...
		await this.logHelper.add('inserted', { executionTime: this.getExecutionTime(timeStart), item }, result);
	...
	async save(item, setOnInsert = {}, options = {}) {
		...
		await this.logHelper.add('upserted', { executionTime: this.getExecutionTime(timeStart), item }, result);
	...
	async update(values, filter, params) {
		...
		await this.logHelper.add('updated', { executionTime: this.getExecutionTime(timeStart), values, filter, params }, isObject(filter) && filter.id);
	...
	async increment(filters, incrementData) {
		...
		await this.logHelper.add('incremented', { executionTime: this.getExecutionTime(timeStart), incrementData, updatedData, result }, result._id.toString()); // eslint-disable-line no-underscore-dangle, max-len
	...
	async remove(item) {
		...
		await this.logHelper.add('removed', { executionTime: this.getExecutionTime(timeStart), item }, item.id);
	...
	async multiInsert(items, options = {}) {
		...
		await this.logHelper.addByItem('inserted', Array.isArray(result) ? result : items, this.getExecutionTime(timeStart));
	...
	async multiSave(items, setOnInsert = {}, options = {}) {
		...
		await this.logHelper.addByItem('upserted', items, this.getExecutionTime(timeStart));
	...
	async multiRemove(filter) {
		...
		await this.logHelper.add('removed', { executionTime: this.getExecutionTime(timeStart), filter }, isObject(filter) && filter.id);
