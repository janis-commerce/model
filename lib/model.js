...
class Model {
	...
	get clientDBConfig() {
		...
		if(typeof this._dbConfig === 'undefined') {
			...
			['read', 'write'].forEach(operationType => {
				this._dbConfig[operationType] = {
					type: this.client[clientFields[operationType].type],
					protocol: this.client[clientFields[operationType].protocol],
					host: this.client[clientFields[operationType].host],
					database: this.client[clientFields[operationType].database],
					user: this.client[clientFields[operationType].user],
					password: this.client[clientFields[operationType].password],
					port: this.client[clientFields[operationType].port]
				};
			});
		}
