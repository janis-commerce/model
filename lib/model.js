...
class Model {
	...
	get dispatcher() {
		...
		if(!this._dispatcher)
			this._dispatcher = this.session ? this.session.getSessionInstance(DatabaseDispatcher) : new DatabaseDispatcher();

		return this._dispatcher;
	}

	getDb() {
		return this.dispatcher.getDb(this.databaseKey, !!this.useReadDB);
	}

	isCore() {
		return this.dispatcher.isCore(this.databaseKey);
	}
	 ...
	 * @param {{ limit: number|undefined}} [params]
	 * @returns {Promise<{ pages: number, page: number, limit: number, total: number }>}
	 */
	async getTotals(filters, params) {
		...
		return db.getTotals(this, filters, params);
	}
	...
	async insert(item) {
		...
		const timeStart = this.startExecutionTime();

		const result = await db.insert(this, item);

		await this._addLog('inserted', { executionTime: this.getExecutionTime(timeStart), item }, result);
	...
	async save(item, setOnInsert = {}, options = {}) {
		...
		if(!options?.skipAutomaticSetModifiedData)
			this.addModifiedData(item); // to standardize write methods (insert(), update(), save(), multiSave())

		const timeStart = this.startExecutionTime();

		const result = await db.save(this, item, setOnInsert, options);

		await this._addLog('upserted', { executionTime: this.getExecutionTime(timeStart), item }, result);
	...
	async update(values, filter, params) {
		...
		await this._addLog('updated', { executionTime: this.getExecutionTime(timeStart), values, filter, params }, isObject(filter) && filter.id);
	...
	async increment(filters, incrementData) {
		...
		await this._addLog('incremented', { executionTime: this.getExecutionTime(timeStart), incrementData, updatedData, result }, result._id.toString()); // eslint-disable-line no-underscore-dangle, max-len
	...
	async remove(item) {
		...
		await this._addLog('removed', { executionTime: this.getExecutionTime(timeStart), item }, item.id);
	 ...
	 * @param {object} options An additional options for the multiInsert operation
	 * @returns {Promise<Object[]>} The items inserted
	 */
	async multiInsert(items, options = {}) {
		...
		const result = await db.multiInsert(this, items, options);

		await this._addLogByItem('inserted', Array.isArray(result) ? result : items, this.getExecutionTime(timeStart));
	...
	async multiSave(items, setOnInsert = {}, options = {}) {
		...
		const result = await db.multiSave(this, items, setOnInsert, options);

		await this._addLogByItem('upserted', items, this.getExecutionTime(timeStart));
	...
	async multiRemove(filter) {
		...
		await this._addLog('removed', { executionTime: this.getExecutionTime(timeStart), filter }, isObject(filter) && filter.id);
	 ...
	 * @param {options{}} options An additional options for the aggregate operation
	...
	async aggregate(stages, options = {}) {
		...
		return db.aggregate(this, stages, options);
	}
	 ...
	 * @param {object} options An additional options for the multiUpdate operation
	 * @returns {Promise<object[]>} The result of the operations
	 */
	async multiUpdate(operations, options = {}) {
		...
		operations.forEach(operation => {
			...
			const skipAutomaticSetModifiedData = operation.options?.skipAutomaticSetModifiedData || options?.skipAutomaticSetModifiedData;
		...
		return db.multiUpdate(this, operations, options);
	}
	...
	/**
	 * Builds and sends the automatic log for a single write operation to the trace service.
	 * The whole logging gate (including core resolution) lives in _shouldCreateLog(), which leaves
	 * this._isCore already resolved so this method only routes the log through the LogHelper.
	 *
	 * @param {string} type The log type
	 * @param {Object} log The log field object
	 * @param {string|Array<string>} [entityId] The entity id (or ids) the log refers to
	 * @returns {Promise<void>}
	 */
	async _addLog(type, log, entityId) {

		if(!await this._shouldCreateLog())
			return;

		return this._getLogHelper().add(type, log, entityId, this._isCore);
	}

	/**
	 * Builds and sends the automatic logs for a multi write operation to the trace service. Items are
	 * grouped into batched logs (each carrying `items` + a parallel `relatedEntities` array of `entity:id` tokens) bounded by item
	 * count and serialized byte budget; a batch that resolves to a single item keeps the legacy
	 * per-item shape (`entityId` + `item`). Same gate and core resolution as _addLog().
	 *
	 * @param {string} type The log type
	 ...
	 * @returns {Promise<void>}
	 */
	async _addLogByItem(type, items, executionTime) {

		if(!await this._shouldCreateLog())
			return;

		return this._getLogHelper().addByItem(type, items, executionTime, this._isCore);
	}

	/**
	 * Whether automatic logs must be created for the next write. Consolidates the full gate:
	 * the static opt-out, the one-shot disable flag (consumed here so logging is restored for
	 * subsequent operations), the core resolution (memoized into this._isCore for the caller to
	 * route on) and the final client/core presence check. When the model has no clientCode in
	 * session and is not core, the log cannot be routed anywhere, so it is discarded with a warn:
	 * this is a bug that must not be silenced.
	 * @returns {Promise<boolean>}
	 * @private
	 */
	async _shouldCreateLog() {

		if(!this.constructor.shouldCreateLogs)
			return false;

		if(this._logsDisabled) {
			this._logsDisabled = false;
			return false;
		}

		await this._resolveIsCore();

		if(!this.session?.clientCode && !this._isCore) {
			logger.warn(`${this.constructor.name} - Discarding log: model has no clientCode in session and is not core. This is likely a bug.`);
			return false;
		}
	...
	/**
	 * Resolves whether the model's databaseKey maps to a core database, memoizing the result. The
	 * databaseKey is fixed per model instance, so a single resolution is enough for its whole lifecycle
	 * and the warm path pays no extra IO (the ParameterStore is already cached by getDb()).
	 * @returns {Promise<boolean>}
	 * @private
	 */
	async _resolveIsCore() {
		...
		if(typeof this._isCore === 'undefined')
			this._isCore = await this.dispatcher.isCoreDatabase(this.databaseKey);

		return this._isCore;
	}

	/**
	 * Lazily builds and memoizes the LogHelper instance for this model. Memoization keeps the custom
	 * log data set via setLogData() alive until the next write consumes it.
	 * @returns {LogHelper}
	 * @private
	 */
	_getLogHelper() {
	...
	setLogData(logMessageOrData) {
		...
		this._getLogHelper().setLogData(logMessageOrData);
	 ...
	 */
	disableLogs() {
		this._logsDisabled = true;
		return this;
	}

	/**
	 * Re-enables the automatic logs, reverting a previous disableLogs() before it is consumed by the
	 * next operation.
	 *
	 */
	enableLogs() {
...
/** @typedef {QueryWithTwoArguments<Array<object>, object, Array<Entity>>} MultiInsert */
/** @typedef {QueryWithOneArgument<Filters, number>} MultiRemove */
/** @typedef {QueryWithThreeArguments<Array<object>, object, object, boolean>} MultiSave */
/** @typedef {QueryWithTwoArguments<Array<UpdateOperation>, object, Array<object>>} MultiUpdate */
/** @typedef {QueryWithOneArgument<object, boolean>} Remove */
/** @typedef {QueryWithThreeArguments<object, object, object, EntityId>} Save */
 ...
 * @property {MultiUpdate} [multiUpdate]
