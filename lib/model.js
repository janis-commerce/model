...
class Model {
	...
	async insert(item) {
		...
		await this.addLog('inserted', { executionTime: this.getExecutionTime(timeStart), item }, result);
	...
	async save(item, setOnInsert = {}, options = {}) {
		...
		await this.addLog('upserted', { executionTime: this.getExecutionTime(timeStart), item }, result);
	...
	async update(values, filter, params) {
		...
		await this.addLog('updated', { executionTime: this.getExecutionTime(timeStart), values, filter, params }, isObject(filter) && filter.id);
	...
	async increment(filters, incrementData) {
		...
		await this.addLog('incremented', { executionTime: this.getExecutionTime(timeStart), incrementData, updatedData, result }, result._id.toString()); // eslint-disable-line no-underscore-dangle, max-len
	...
	async remove(item) {
		...
		await this.addLog('removed', { executionTime: this.getExecutionTime(timeStart), item }, item.id);
	...
	async multiInsert(items, options = {}) {
		...
		await this.addLogByItem('inserted', Array.isArray(result) ? result : items, this.getExecutionTime(timeStart));
	...
	async multiSave(items, setOnInsert = {}, options = {}) {
		...
		await this.addLogByItem('upserted', items, this.getExecutionTime(timeStart));
	...
	async multiRemove(filter) {
		...
		await this.addLog('removed', { executionTime: this.getExecutionTime(timeStart), filter }, isObject(filter) && filter.id);
	 ...
	 * @returns {Promise<void>}
	 */
	async addLog(type, log, entityId) {
	 ...
	 * using each item id as entityId. Same gate and core resolution as addLog().
	 *
	 * @param {string} type The log type
	 ...
	 * @returns {Promise<void>}
	 */
	async addLogByItem(type, items, executionTime) {
