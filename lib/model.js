...
class Model {
	...
	get dateToSave() {
		...
		if(!this._dateToSave)
			this._dateToSave = new Date();

		return this._dateToSave;
	}
	...
	setExecutionStart() {
		this.executionStart = process.hrtime();
	}

	get executionTime() {
		return process.hrtime(this.executionStart)[1] / 1000000;
	}
	...
	async insert(item) {
		...
		item.userCreated = this.userToSave;
		item.dateCreated = this.dateToSave;

		// to standardize write methods (insert(), update(), save(), multiSave())
		item.userModified = this.userToSave;
		item.dateModified = this.dateToSave;
	...
	async save(item, setOnInsert = {}) {
		...
		setOnInsert = {
			...setOnInsert,
			userCreated: this.userToSave,
			dateCreated: this.dateToSave
		};
	...
	async update(values, filter, params) {
		...
		if(Array.isArray(values)) {
			values.push({
				userModified: this.userToSave,
				dateModified: this.dateToSave
			});
		} else {
			values.userModified = this.userToSave;
			values.dateModified = this.dateToSave;
		}
	...
	async multiInsert(items) {
		...
		await this.logHelper.addByItem('inserted', items, this.executionTime);
