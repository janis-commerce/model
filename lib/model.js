...
class Model {
	 ...
	 * Applies the logging gate, resolves whether the model maps to a core database (memoized) and
	 * delegates the log building/sending to the LogHelper with the resolved `isCore`.
	 *
	...
	async _addLog(type, log, entityId) {
		...
		return this._getLogHelper().add(type, log, entityId, isCore);
	}
	...
	async _addLogByItem(type, items, executionTime) {
		...
		return this._getLogHelper().addByItem(type, items, executionTime, isCore);
	}

	/**
	 * Whether automatic logs must be created for the next write. Applies the static opt-out and the
	 * one-shot disable flag, which is consumed here so logging is restored for subsequent operations.
	 * @returns {boolean}
	 * @private
	 */
	_shouldCreateLog() {
	...
	setLogData(logMessageOrData) {
		...
		this._getLogHelper().customLogData = {
