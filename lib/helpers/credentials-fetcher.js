...
module.exports = class CredentialsFetcher {
	...
	static async fetch(config) {
		...
		try {
			...
			if(secretValue
				&& secretValue.databases
				&& secretValue.databases[config.databaseKey]
				&& secretValue.databases[config.databaseKey][accessType])
				credentials = secretValue.databases[config.databaseKey][accessType];
