...
module.exports = class LogHelper {
	...
	constructor(params) {
		...
		/**
		 * Core models (databaseKey === 'core') log entities without a client, using the sentinel
		 * client resolved by Log.addCore(). They must log even without a client session.
		 * @private
		 */
		this.isCore = params.databaseKey === 'core';

		/** @private */
		this.shouldCreateLogs = params.shouldCreateLogs && (!!this.session?.clientCode || this.isCore);
