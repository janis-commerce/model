...
// parallel entityids array and the record envelope, so a grouped record never reaches truncation.
 ...
 * @property {Array<string>} [entityids] Parallel entity ids for grouped logs (replaces entityId)
 * @property {string} type
 * @property {string} [message]
 * @property {string} [userCreated]
 * @property {*} [log]
 * @property {Array<Object>} [items] Grouped item payloads (shipped inside `log` for grouped logs)
 */
...
module.exports = class LogHelper {
		...
		if(itemsWithId.length) {
			...
			chunkedItems.forEach((chunk, index) => {
				...
				builtLogs.push(this._buildLog({
					...
					items: chunk
				}, type, undefined, chunk.map(item => item.id)));
			});
		}
	 ...
	 * @param {LogObject} log
	 * @param {string} type
	 * @param {string} [entityId] The singular entity id (individual logs only)
	 * @param {Array<string>} [entityids] The parallel entity ids (grouped logs only)
	 * @return {Log}
	 * @private
	 */
	_buildLog(log, type, entityId, entityids) {
