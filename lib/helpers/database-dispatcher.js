...
module.exports = class DatabaseDispatcher {
	...
	/**
	 * Resolves whether the databaseKey maps to a core database, consulting ONLY the coreDatabases map
	 * from ParameterStore (the deprecated settings path is intentionally excluded to avoid treating
	 * legacy `database.default` settings as core and mis-routing logs).
	 * @param {string} key Database Key
	 * @returns {Promise<boolean>}
	 */
	async isCoreDatabase(key) {
		await this.setParameterStore();
		return !!ParameterStore.getCoreDatabase(key);
	}
