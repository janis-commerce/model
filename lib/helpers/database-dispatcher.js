...
module.exports = class DatabaseDispatcher {
	...
	getDatabaseByConfig(dbConfig, key, useReadDB, adminAccess) {
		...
		if(useReadDB && dbConfig.read)
			...
			targetDbConfig = dbConfig.write; // admin usa credenciales propias pero la conexion de write
	...
	async getDatabaseFromCache(dbConfig) {
		...
		if(!this.constructor.databasesCache) {
			/** @private */
			this.constructor.databasesCache = {};
		}

		const cacheKey = md5(JSON.stringify(dbConfig));

		if(!this.constructor.databasesCache[cacheKey])
			this.constructor.databasesCache[cacheKey] = this.buildDatabaseCacheCache(dbConfig);

		return this.constructor.databasesCache[cacheKey];
	}

	async buildDatabaseCacheCache(dbConfig) {

		if(this.shouldFetchCredentials(dbConfig))
			dbConfig = await CredentialsFetcher.fetch(dbConfig);

		return this.getDBDriver(dbConfig);
	}
