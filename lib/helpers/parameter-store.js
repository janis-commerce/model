...
module.exports = class ParameterStore {
	...
	static set() {
		...
		this._setPromise = this._fetch();

		return this._setPromise;
	}

	/**
	 * Fetches the ParameterStore parameter and caches its value. Kept separate from set() so the
	 * memoization stays readable: set() only decides whether to reuse the in-flight promise, this
